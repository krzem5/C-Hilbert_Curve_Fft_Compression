...
void compress_matrix(uint8_t* bf,compressed_data_t* o){
	uint8_t dt[MATRIX_SIZE*MATRIX_SIZE];
	for (uint16_t i=0;i<MATRIX_SIZE*MATRIX_SIZE;i++){
		...
		dt[i]=*(bf+y*MATRIX_SIZE+x);
	}
	epicycle_t e[MATRIX_SIZE*MATRIX_SIZE-1];
	uint16_t i=0;
	float a=0;
	for (uint16_t j=0;j<MATRIX_SIZE*MATRIX_SIZE;j++){
		...
		e[i].r=re*re+im*im;
		if (e[i].r>=MIN_AMPLITUDE*MIN_AMPLITUDE*MATRIX_SIZE*MATRIX_SIZE*MATRIX_SIZE*MATRIX_SIZE){
			e[i].r=sqrtf(e[i].r)/(MATRIX_SIZE*MATRIX_SIZE);
			e[i].s=atan2f(-im,re);
			e[i].f=j;
			i++;
		}
